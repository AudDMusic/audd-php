...
Official PHP SDK for [AudD](https://audd.io) — music recognition from a short audio clip, a long audio file, or a live stream.
...
```bash
composer require audd/audd:^1.4.5
```
...
```php
$audd = new AudD(apiToken: 'your-token');
```
...
Valid `return_` values: `apple_music`, `spotify`, `deezer`, `napster`, `musicbrainz`. The corresponding properties (`$result->apple_music`, `$result->spotify`, …) are `null` when not requested.
...
The callback receives JSON; parse it into a typed payload inside your webhook handler:

```php
// Given the parsed JSON body in $body:
$payload = $audd->streams()->parseCallback($body);
if ($payload->isResult()) {
    foreach ($payload->result->results as $entry) {
        echo $entry->artist, ' — ', $entry->title, ' score=', $entry->score, "\n";
    }
} elseif ($payload->isNotification()) {
    echo 'notification: ', $payload->notification->notification_message, "\n";
}
```
...
foreach ($audd->streams()->longpoll($category, timeout: 30) as $event) {
    print_r($event);  // raw array — same shape as a callback payload
}
```
...
foreach ($consumer->iterate(timeout: 30) as $event) {
    print_r($event);
}
```
