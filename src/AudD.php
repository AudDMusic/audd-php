...
final class AudD
{
    ...
    {
        // custom_catalog.add is metered — never auto-retry; a transport
        // failure could double-charge for the same fingerprinting work.
     ...
     *
     * @param string|list<string>|null $return_metadata  Comma-string or list of metadata keys:
     *                                           apple_music, spotify, deezer, napster, musicbrainz.
     * @param array<string, string>    $extra_parameters Additional form fields
     *                                                   the typed params don't cover. Typed params win on collision.
     */
    public function recognize(
        mixed $source,
        string|array|null $return_metadata = null,
        ?string $market = null,
        ?float $timeout = null,
        ...
        $do = function () use ($reopen, $ret, $market, $timeout, $extra_parameters): \AudD\Internal\HttpResponse {
            [$data, $files] = $reopen();
            // extra_parameters first; typed params win on collision.
     ...
     * @phpstan-param string|StreamInterface|SourceBytes|resource $source
     *
     * @param string|list<string>|null $return_metadata
     * @param array<string, string>    $extra_parameters Additional form fields
     *                                                   the typed params don't cover. Typed params win on collision.
     *
    ...
    public function recognizeEnterprise(
        ...
        $do = function () use ($reopen, $extra, $timeout, $extra_parameters): \AudD\Internal\HttpResponse {
            [$data, $files] = $reopen();
            // extra_parameters first; typed fields override on collision.
            foreach ($extra_parameters as $k => $v) {
                $data[$k] = $v;
            }
    ...
    private function noRetryPolicy(): Retry
    {
        // maxAttempts=1 → exactly one attempt, no retries. Used for metered
        // mutations where re-running silently could double-charge the user.
        return new Retry(RetryClass::NONE, maxAttempts: 1, backoffFactor: $this->backoffFactor);
    }

    /**
     * @param string|list<string>|null $return_metadata
     */
    private static function formatReturn(string|array|null $return_metadata): ?string
    {
        if ($return_metadata === null) {
            return null;
        }
        if (is_string($return_metadata)) {
            return $return_metadata;
        }
        return implode(',', $return_metadata);
    }
